...
class Adafruit_BME680:
  ...
  def humidity(self):
    ...
    if calc_hum < 0:
      calc_hum = 0
    return calc_hum
  def humidity_score (self,humidity):
    hum_reference = 40
    if (humidity >= 38) and (humidity <= 42):
      humidity_score = 0.25 * 100
    if humidity < 38:
      humidity_score = 0.25 / hum_reference * humidity * 100
    else:
      humidity_score = ((-0.25 / (100 - hum_reference) * humidity) + 0.416666) * 100
    print ( 'Humidity score ' + str( round(humidity_score,1) ) )
  ...
  def gas(self):
    ...
    print ( 'Gas ' + str( round(calc_gas_res,1) ) )
  ...
  def gas_score(self,gas):
    ...
    if gas_score < 0:
        gas_score = 0
    print ( 'Gas score ' + str( round(gas_score,1) ) )
    return gas_score
...
class BME680_I2C(Adafruit_BME680):
  ...
  @property
  def iqa_score(self):
      score = self.gas_score(self.gas) + self.humidity_score(self.humidity)
      score = (100 - score) * 5
      return float(score)
  def iqa(self):
